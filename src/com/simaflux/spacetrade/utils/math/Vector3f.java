...
public class Vector3f implements Serializable {
	...
	private static final long serialVersionUID = -4181699882663611552L;
    ...
    public Vector3f mod(float mod) {
    	return new Vector3f(x % mod, y % mod, z % mod);
    }
    
    /*
     * If the values in the vector is more than half the entered number, it will subtract the number
     */
    public Vector3f cent(float num) {
    	
    	float tx = (x > num / 2f) ? x - num : x;
    	tx = (x < - num / 2f) ? x + num : tx;
    	float ty = (y > num / 2f) ? (y - num) : y;
    	ty = (y < - num / 2f) ? y + num : ty;
    	float tz = (z > num / 2f) ? z - num : z;
    	tz = (z < - num / 2f) ? z + num : tz;
